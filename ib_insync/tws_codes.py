...
'''
    Return codes for TWS API
'''


systemMessageCodes = {
    "":"\033[1;34mSystem Message Codes\033[0;39:49m",
...
warningMessageCodes = {
    "\033[1;34m":"Warning Message Codes\033[0;39:49m",
	"2100":"New account data requested from TWS. API client has been unsubscribed from account data.  ::: The TWS only allows one https://interactivebrokers.github.io/tws-api/classIBApi_1_1EClient.html#aea1b0d9b6b85a4e0b18caf13a51f837f (IBApi.EClient.reqAccountUpdates) request at a time. If the client application attempts to subscribe to a second account without canceling the previous subscription, the new request will override the old one and the TWS will send this message notifying so.",
	"2101":"Unable to subscribe to account as the following clients are subscribed to a different account.  ::: If a client application invokes https://interactivebrokers.github.io/tws-api/classIBApi_1_1EClient.html#aea1b0d9b6b85a4e0b18caf13a51f837f  (IBApi.EClient.reqAccountUpdates) when there is an active subscription <b>started by a different client</b>, the TWS will reject the new subscription request with this message.",
	"2102":"Unable to modify this order as it is still being processed.  ::: If you attempt to modify an order before it gets processed by the system, the modification will be rejected. Wait until the order has been fully processed before modifying it. See https://interactivebrokers.github.io/tws-api/order_submission.html (Placing Orders) for further details.",
	...
	"2158":"Sec-def data farm connection is OK  ::: A notification that connection to the Security definition data server is ok. This is a notification and <b>not</b> a true error condition, and is expected on first establishing connection.",
	...
	"2168":"Etrade Only Not Supported Warning  ::: The EtradeOnly https://interactivebrokers.github.io/tws-api/classIBApi_1_1Order.html (IBApi.Order) attribute is no longer supported. Error received with TWS versions 983+. Remove attribute to place order.",
	"2169":"Firm Quote Only Not Supported Warning  ::: The firmQuoteOnly https://interactivebrokers.github.io/tws-api/classIBApi_1_1Order.html (IBApi.Order) attribute is no longer supported. Error received with TWS versions 983+. Remove attribute to place order.",
    }



clientErrorCodes = {
    "":"\033[1;34mClient Error Codes\033[0;39:49m",
	"501":"Already Connected.   :::   Your client application is already connected to the TWS.",
	"502":"Couldn't connect to TWS. Confirm that 'Enable ActiveX and Socket Clients' is enabled and connection port is the same as 'Socket Port' on the TWS 'Edit->Global Configuration...->API->Settings' menu.    :::   When you receive this error message it is either because you have not enabled API connectivity in the TWS and/or you are trying to connect on the wrong port. Refer to the TWS' API Settings as explained in the error message.  See also https://interactivebrokers.github.io/tws-api/connection.html",
	...
	"10038":" WinError ::: An operation was attempted on something that is not a socket. This indicates that the socket connection was closed improperly. Typically for Python clients. You may refer to  https://stackoverflow.com/questions/15210178/python-socket-programming-oserror-winerror-10038-an-operation-was-attempted-o",
    }


twsErrorCodes = {
    "\033[1;34m":"TWS Error Codes\033[0;39:49m",
	...
	"144":"Order size does not match total share allocation. To adjust the share allocation, right-click on the order and select odify; Share Allocation.â€�  :::",
	...
	"200":"The contract description specified for &lt;Symbol&gt; is ambiguous  ::: Ambiguity may occur when the contract definition provided is not unique.  For some stocks that has the same Symbol, Currency and Exchange, you need to specify the IBApi.Contract.PrimaryExch attribute to avoid ambiguity.  For futures that has multiple multipliers for the same expiration, You need to specify the IBApi.Contract.Multiplier attribute to avoid ambiguity.",
	...
	"10148":"OrderId &lt;OrderId&gt; that needs to be cancelled can not be cancelled, state: ::: An attempt was made to cancel an order that had already been filled by the system.",
	...
	"10231":"The following Groups and/or Profiles contain invalid accounts: &lt;list of groups/profiles&gt; ::: If the account(s) inside Groups or Profiles is/are incorrect in xml-formatted configuration string of replaceFA request, then the error shows list of such Groups and/or Profiles.",
